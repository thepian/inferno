...
const browserHistoryWithBaseName = createBrowserHistory({
...
describe("Router (jsx)", () => {
        ...
        <Router url={"/foo/bar"} history={browserHistoryWithBaseName}>
          ...
          >
            <Route
              ...
              component={({ params }) =>
                <div>
                  Child is {params.test} Link is <Link to="/foo/test" />
                </div>}
            />
          </Route>
        </Router>,
  ...
  describe("#history", () => {
    it("should render the parent component only", () => {
      ...
      expect(container.innerHTML).toBe(
        innerHTML("<div><p>Parent Component</p></div>")
      );
    });
    it("should render the child and inherit parent (partial URL)", () => {
      ...
      expect(container.innerHTML).toBe(
        innerHTML("<div><p>Parent Component</p><div>Child is bar</div></div>")
      );
    });
    ...
    it("should render the child and inherit parent (URL containing percent encoded value)", () => {
      ...
      expect(container.innerHTML).toBe(
        innerHTML("<div><p>Parent Component</p><div>Child is 100%</div></div>")
      );
    });
    it("should render the child and inherit parent (URL search param containing percent encoded value)", () => {
      ...
      expect(container.innerHTML).toBe(
        innerHTML("<div><p>Parent Component</p><div>Child is 50%</div></div>")
      );
    });
    ...
    it("should render the TestComponent with given paths", () => {
      render(createRouterWithSingleRoute("/", "/", TestComponent), container);
    ...
    it("should render the TestComponent with the highest ranked path", () => {
      render(
        <Router url={"/foo/bar/yar"} history={browserHistory}>
          ...
          <Route
            path={"/foo/bar/*"}
            component={() => <div>Bad Component</div>}
          />
    ...
    it("should render the correct nested route based on the path", () => {
      ...
      expect(container.innerHTML).toBe(
        innerHTML("<div><div>Good Component</div></div>")
      );
    });
    it("should render the both components with same params prop passed down", () => {
      ...
      expect(container.innerHTML).toBe(
        innerHTML("<div><div>Param is bar</div></div>")
      );
    });
    ...
    it("should map through routes with new props", () => {
      ...
      expect(container.innerHTML).toBe(
        innerHTML("<div>Good Component Clone</div>")
      );
    });
  ...
  describe("#match", () => {
    it("should find route when url has normal value", () => {
      ...
      expect(actual.props.matched.props.children.props.component).toBe(
        BadComponent
      );
      expect(
        actual.props.matched.props.children.props.children.props.component
      ).toBe(GoodComponent);
    });
